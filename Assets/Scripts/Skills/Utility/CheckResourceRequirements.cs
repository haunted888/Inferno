...
public class CheckResourceRequirements : Skill
{
    ...
    public override void Execute(BattleCharacter user, BattleCharacter target)
    {
        ...
        } else if (skillReference.skillDetailShell.ammoCost + skillReference.skillDetailShell.flatAmmoCost > 0)
        {
            EndExecution();
            return; // Skill requires ammo but user is not a marksman, so the skill fails
         }
